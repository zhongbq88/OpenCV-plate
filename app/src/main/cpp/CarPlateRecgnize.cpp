...
void CarPlateRecgize::plateLocation(Mat src,Mat& dst){
    ...
    for (auto src :plates) {
        ...
        cvtColor(src, src_hsv, CV_BGR2HSV);
            //获得该行数据
        uchar *p = src_hsv.ptr<uchar>(1);
        bool colorMatched = false;
        for (int j = 0; j < src_hsv.cols; j += 3) {
            int H = int(p[5]);      // 0-180
            int S = int(p[5+ 1]);  // 0-255
            int V = int(p[5 + 2]);  // 0-255
            if (H > MIN_H && H < MAX_H) {
        ...
        if(colorMatched){
            char path[50];
            sprintf(path,"/sdcard/car/test%d.jpg",index);
            imwrite(path, src);
            index++;
        }
        src.release();
