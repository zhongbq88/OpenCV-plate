...
CarColorPlateMatches *matches=0;
...
Java_com_lichao_opencv_1carplate_MainActivity_init(JNIEnv *env, jobject instance, jstring svm_,
    ...
    matches = new CarColorPlateMatches();
...
Java_com_lichao_opencv_1carplate_MainActivity_matches(JNIEnv *env, jobject instance,
                                                          jobject bitmap, jobject out) {
    Mat src;
    Mat dst;
    bitmap2Mat(env, bitmap, src);
    bitmap2Mat(env, out, dst);
    int str = matches->matches(src, dst);
    src.release();
    dst.release();
    return str;
}
